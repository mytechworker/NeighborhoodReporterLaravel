@extends('layouts.app')
@section('title')
Map Neighborhood Reporter
@endsection
@section('meta')
<!-- Primary Meta Tags -->
<meta name="title" content="Neighborhood Reporter is a network of hyperlocal news sites spread across the United States.">
<meta name="description" content="All Neighborhood Reporter Locations by State | Neighborhood Reporter">
<!-- Open Graph / Facebook -->
<meta property="og:locale" content="{{str_replace('_', '-', app()->getLocale())}}">
<meta property="og:type" content="article">
<meta property="og:url" content="{{route('home')}}/map">
<meta property="og:title" content="Neighborhood Reporter is a network of hyperlocal news sites spread across the United States.">
<meta property="og:description" content="All Neighborhood Reporter Locations by State | Neighborhood Reporter">
...
<meta property="twitter:url" content="{{route('home')}}/map">
<meta property="twitter:title" content="Neighborhood Reporter is a network of hyperlocal news sites spread across the United States.">
<meta property="twitter:description" content="All Neighborhood Reporter Locations by State | Neighborhood Reporter">
...
<div class="content-sec">
    <div class="container pad-0">
        <div>
            <section class="position-div">
                <a id="main-content"></a>
                <div class="region region-content">
                    <section id="block-fe-components-fe-components-header-block" class="block block-fe-components clearfix">
                        <div id="header-block" class="clearfix standard-header">
                            <h1 class="text-center h3">Neighborhood reporter is a network of hyper local news sites spread across the United States.</h1> 
                            <div class="clearfix summary-additional">
                                <div class="text-center">We're currently in <span class="h5 l-bold">1255</span> communities and expanding every day. Want a Neighborhood reporter in your town? Email <a href="mailto:suggest@neighborhoodreporter.com">suggest@neighborhoodreporter.com</a>
                                    <br>To send a press release or a news tip, please refer to the directory below for a list of all Neighborhood reporters to find your local page, then click the envelope icon at the bottom of that page.</div>
                            </div>
                        </div>
                    </section>
                    ...
                    <ul class="list-of-states">
                        ...
                        <div class="map-name">
                            <li data-state="{{$value['region_code']}}" class="region-li">
                                <a href="/r/{{sanitizeStringForUrl($value['name'])}}">{{$value['name']}} ({{count($value['community'])}})</a>
                                <div class="faplus"> <span>+</span>
                                </div>
                                <div class="region-wrapper map-sub-name">
                                    <div class="patches-wrapper" id="region-patch-{{$value['region_code']}}">
                                        @if(count($value['community']) > 0)
                                        <ul class="list-unstyled list-columned">
                                            @foreach($value['community'] as $cvalue)
                                            <li>
                                                <a href="/l/{{sanitizeStringForUrl($value['name'])}}/{{sanitizeStringForUrl($cvalue['name'])}}">{{$cvalue['name']}}</a>
                                            </li>
                                            @endforeach
                                        </ul>
                                        @endif 
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        //BEGIN
        $(document).on("click", "#states path,#states circle", function () {
            $('.region-li').removeClass('border-li');
            $('.faplus').removeClass('faq__active');
            $('.region-wrapper').removeClass('show-map');
            var stateId = $(this).attr('id');
            document.querySelector("[data-state='" + stateId + "']").classList.add('border-li');
            $("[data-state='" + stateId + "']").find('.faplus').addClass('faq__active');
            $("[data-state='" + stateId + "']").find('.region-wrapper').addClass('show-map');
            document.querySelector("[data-state='" + stateId + "']").scrollIntoView({
                behavior: 'smooth',
                inline:'center',
                block:'center'
            });
        ...
        $(document).on("click", ".faplus", function (e) {
            ...
            if (!$this.hasClass("faq__active")) {
                $(".region-wrapper map-sub-name").show();
                $(".js-faq-title").removeClass("faq__active");
            }
            $(this).parent().toggleClass('border-li');
            $this.toggleClass("faq__active");
            $this.next().toggleClass('show-map');
