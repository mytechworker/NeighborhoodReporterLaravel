...
use App\Mail\LocalClassifiedMail;
use Carbon\Carbon;

class LocalclassifiedCommand extends Command {
     ...
     * @var string
     */
    protected $signature = 'localclassified:command';
     ...
     * @var string
     */
    protected $description = 'Localclassified Command';
    ...
    public function handle() {
        ...
        foreach ($users as $user) {
            ...
            foreach ($user_communities as $user_community) {
                $user_setting = SettingEmail::where('user_id', $user_community->user_id)->where('community_id', $user_community->communitie_id)->where('classifieds', '1')->count();
                if ($user_setting == 1) {
                    ...
                    $classifieds = Classified::where('location', $user_region->name)
                            ...
                            ->whereBetween('created_at', [Carbon::now()->subDays(7)->format('Y-m-d H:i:s'), date('Y-m-d H:i:s')])
                    ...
                    if (count($classifieds) > 0) {
                        ...
                        Mail::to($user->email)->send(new LocalClassifiedMail($data));
                    }
                }
            }
        }
